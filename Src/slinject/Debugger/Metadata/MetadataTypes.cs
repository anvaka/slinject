...
namespace slinject.Debugger.Metadata
{
    ...
    public class AscendingValueComparer<K, V> : IComparer<KeyValuePair<K, V>> where V : IComparable
    {
    ...
    public class TypeDefEnum : IEnumerable, IEnumerator, IDisposable
    {
        public TypeDefEnum(DebugMetadataImporter corMeta)
        {
            _importer = corMeta;
        }
        ...
        public bool MoveNext()
        {
            ...
            _importer.Importer.EnumTypeDefs(ref m_enum, out token, 1, out c);
            if (c == 1) // 1 new element
                m_type = _importer.GetType(token);
            else
                m_type = null;
            return m_type != null;
        }
        ...
        {
            _importer.Importer.CloseEnum(m_enum);
            m_enum = new IntPtr();
        }

        private DebugMetadataImporter _importer;
